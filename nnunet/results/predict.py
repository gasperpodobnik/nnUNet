...
class Custom_nnUNet_Predict(object):
    ...
    def get_out_dir(self):
        if self.out_dir is None:
            self.out_dir = join(self.base_nnunet_dir_on_medical, self.model_task_name, self.results_dir_name)
        elif not self.out_dir.startswith('/'):
            self.out_dir = join(self.base_nnunet_dir_on_medical, self.model_task_name, self.results_dir_name, self.out_dir)
        os.makedirs(self.out_dir, exist_ok=True)
        logging.info(f'Using output directoty {self.out_dir}')
    ...
    def predict_one_by_one(self):
        ...
        for self.phase, self.img_dir, _dict_tmp in self.iterate_image_source_dirs():
            ...
            case_ids = [cid for cid in case_ids.tolist() if self.inverse_splits_final_dict[cid] == self.phase]

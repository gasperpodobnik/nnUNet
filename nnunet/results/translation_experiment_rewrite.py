...
import nnunet.results.predict_rewrite as predict_rewrite
...
class nnUNet_Registration_Arg_Parser(predict_rewrite.nnUNet_Prediction_Arg_Parser):
...
class Patch_Registration_nnUNet_Predict(predict_rewrite.Custom_nnUNet_Predict):
    ...
    def reg_patch_and_predict(self):
        ...
        for preprocessed in self.preprocessing:
            ...
            for self.organ_of_interest in self.organs_of_interest:
                enum, d, dct = self.register_patch(d, dct)
    ...
    def append_to_successful_list(self, fname, pred_fpath, gt_fpath):
        super().append_to_successful_list(fname, pred_fpath, gt_fpath)
        self.successfully_predicted['rigid'] = self.isRigid
        self.successfully_predicted['deformable'] = self.isDeformable
        self.successfully_predicted['organ_of_interest'] = self.organ_of_interest
            
    def register_patch(self, d, dct):
        d = np.copy(d)
        dct = copy.deepcopy(dct)
        
        root_results_dir = join(self.out_dir, f'rigid-{self.rigid}_deformable-{self.deformable}_{self.organ_of_interest}')
        ...
        organ_lbl_int = self.organs_labels_dict[self.organ_of_interest]
        
        
        filename = Path(self.gt_filename).name.split('_')[-1].split('.')[0]
        ...
        rigid_transform = fcn.register_patches(fixed_patch=ct_rigid_reg_patch_sitk, moving_patch=mr_rigid_reg_patch_sitk, rigid=self.rigid, deformable=False, root_dir=root_results_dir)
        ...
        for enum, (mr_patch, self.isRigid, self.isDeformable) in enumerate(to_predict):
            ...
            self.output_filename = filepath_root + f'_RED-SEG_rigid-{self.isRigid}_deformable-{self.isDeformable}.nii.gz'
    ...
    def append_case_specific_info_to_settings_info(self):
        ...
        self.settings_info['organ_of_interest'] = self.case['organ_of_interests']

...
class STN_block(nn.Module):
    def __init__(self, input_feature_channels, level):
        ...
        ops_list = [
            nn.Conv3d(self.input_feature_channels, self.input_feature_channels, kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1), **conv_kwargs),
            nn.ReLU(True),
            nn.Conv3d(self.input_feature_channels, self.input_feature_channels, kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1), **conv_kwargs),
            nn.ReLU(True),
            nn.Conv3d(self.input_feature_channels, self.input_feature_channels, kernel_size=(3,3,3), stride=(2,2,2), padding=(1,1,1), **conv_kwargs),
            nn.ReLU(True),
            nn.Conv3d(self.input_feature_channels, self.input_feature_channels, kernel_size=(3,3,3), stride=(2,2,2), padding=(1,1,1), **conv_kwargs),
            nn.ReLU(True)
        ]
    ...
    def forward(self, x_fixed, x_moving):
        ...
        x_moving_reg = F.grid_sample(x_moving, grid)
        # x_moving_reg = x_moving
        return torch.cat([x_fixed, x_moving_reg], dim=1)
...
class Generic_UNet_separate_encoders_STN(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(Generic_UNet_separate_encoders_STN, self).__init__()
        ...
        for d in range(num_pool):
            ...
            self.STN_fusion_blocks.append(STN_block(output_features*self.modality_num, level=d))
        ...
        self.STN_fusion_blocks = nn.ModuleList(self.STN_fusion_blocks)
    ...
    def forward(self, x):
        ...
        for d in range(len(self.conv_blocks_context[0])):
            ...
            skips.append(self.STN_fusion_blocks[d](*x))
            if not self.convolutional_pooling:
                for channel in range(self.modality_num):

...
def download_and_install_pretrained_model_by_name(taskname):
    ...
    if len(av_models[taskname]["url"]) == 0:
        raise RuntimeError(
            "The requested model has not been uploaded yet. Please check back in a few days"
        )
    ...
    if isinstance(url, str):
        ...
        raise RuntimeError(
            "URL for download_and_install_from_url must be either str or list/tuple of str"
        )
...
def download_by_name():
    ...
    parser = argparse.ArgumentParser(
        description="Use this to download pretrained models. CAREFUL: This script will "
        "overwrite "
    ...
    parser.add_argument(
        "task_name",
        type=str,
        help="Task name of the pretrained model. To see "
...
def print_pretrained_model_requirements():
    ...
    parser = argparse.ArgumentParser(
        description="Use this to see the properties of a pretrained model, especially "
    ...
    if taskname not in av.keys():
        raise RuntimeError(
            "Invalid task name. This pretrained model does not exist. To see available task names, "

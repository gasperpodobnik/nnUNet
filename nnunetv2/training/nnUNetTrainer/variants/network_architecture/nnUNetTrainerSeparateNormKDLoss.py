...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_CE_and_KD_loss, KD_loss
...
class nnUNetTrainerSeparateNormKDLossOnOutput(nnUNetTrainer_noMirroringAxis2redRot):
    def _build_loss(self):
        loss = DC_and_CE_and_KD_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, len(self.dataset_json['labels']), {}, weight_ce=1, weight_dice=1,
                                  ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
        kd_loss = KD_loss(len(self.dataset_json['labels']))
        ...
        # now wrap the loss
        loss = DeepSupervisionWrapperKDseparate(loss, kd_loss, weights)
        return loss

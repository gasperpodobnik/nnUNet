...
class nnUNetTrainer(object):
    ...
    def build_network_architecture(
        ...
        # do initialization of lazy modules (this is not done automatically)
        if configuration_manager.UNet_class_name in ["PlainConvUNetSTN"]:
            network(
                torch.rand(
                    (
    ...
    def on_epoch_end(self):
        ...
        pseudo_dice = {
            lbl: val
            for lbl, val in zip(
                list(self.dataset_json["labels"].keys())[1:],
    ...
    def load_checkpoint(
        self, filename_or_checkpoint: Union[dict, str], strict=True
    ) -> None:
        if not self.was_initialized:
            self.initialize()
        ...
        if self.is_ddp:
            if isinstance(self.network.module, OptimizedModule):
                ...
                self.network.module.load_state_dict(new_state_dict, strict=strict)
        else:
